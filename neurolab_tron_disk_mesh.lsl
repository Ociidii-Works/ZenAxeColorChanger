...
integer colorRoot = 0;          // Needed for checking if we want to recolor the root prim
...
doColor(vector dcolor)
{
    ...
    if(listLen > 0)
    {
        // Probably can be re-written using a loop that can exclude some faces
            llSetLinkPrimitiveParamsFast(llList2Integer(foundPrims, 1),
                [PRIM_COLOR,2,dcolor,1.0,
                PRIM_GLOW,2,glowAmount
                ]);
            llSetLinkPrimitiveParamsFast(llList2Integer(foundPrims, 2),
                [PRIM_COLOR,1,dcolor,1.0,
                PRIM_GLOW,1,glowAmount
                ]);
            llSetLinkPrimitiveParamsFast(llList2Integer(foundPrims, 2),
                [PRIM_COLOR,2,dcolor,1.0,
                PRIM_GLOW,2,glowAmount
                ]);
                        llSetLinkPrimitiveParamsFast(llList2Integer(foundPrims,3),
                [PRIM_COLOR,4,dcolor,1.0,
                PRIM_GLOW,4,glowAmount
                ]);
            llSetLinkPrimitiveParamsFast(llList2Integer(foundPrims, 3),
                [PRIM_COLOR,5,dcolor,1.0,
                PRIM_GLOW,5,glowAmount
                ]);
    }
}
